...
class Iseed
{
    ...
    public function getData($table, $max, $exclude = null, $orderBy = null, $direction = 'ASC')
    {
        ...
        if (!empty($exclude)) {
            $allColumns = \DB::connection($this->databaseName)->table($table)->first();
            $allColumns = get_object_vars($allColumns);
            $allColumns = collect($allColumns)->keys()->toarray();
            //dd($allColumns)->tosrray();
    ...
    public function populateStub($class, $stub, $table, $data, $chunkSize = null, $prerunEvent = null, $postrunEvent = null, $indexed = true)
    {
        ...
        foreach ($chunks as $chunk) {
            ...
            foreach ($chunk as $key=>$c) {
                if (isset($chunk[$key]['pzje'])) {
                    $chunk[$key]['pzje'] = bcdiv($chunk[$key]['pzje'], 1,2);
                    
                } elseif(isset($chunk[$key]['je'])) {
                    $chunk[$key]['je'] = bcdiv($chunk[$key]['je'], 1,2);
                }     
            }
